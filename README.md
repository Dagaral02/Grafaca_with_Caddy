...
```yaml
- alert: jenkins_down
    expr: absent(container_memory_usage_bytes{name="JV-Z1"})
    for: 30s
    labels:
      severity: critical
    annotations:
...
- alert: jenkins_high_cpu
    expr: sum(rate(container_cpu_usage_seconds_total{name="JV-Z1"}[1m])) / count(node_cpu_seconds_total{mode="system"}) * 100 > 10
    for: 30s
    labels:
      severity: warning
    annotations:
...
- alert: jenkins_high_memory
    expr: sum(container_memory_usage_bytes{name="JV-Z1"}) > 1200000000
    for: 30s
    labels:
      severity: warning
    annotations:
